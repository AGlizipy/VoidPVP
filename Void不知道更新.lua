...
local Translations = {
    ...
    ["English"] = {
        ...
        ["人数: "] = "Players: ",
        ["赏金: "] = "Bounty: ",
        ["Rayfield ui版脚本链接"] = "Rayfield UI script link",
        ["点击复制"] = "Click to copy",
        ["已复制链接"] = "Link copied",
        ["选择人数"] = "Select Players",
...
task.spawn(function()
    ...
    local colorC = Color3.fromRGB(135, 206, 235)
    local colorD = Color3.fromRGB(255, 183, 197)
    ...
    runService.Heartbeat:Connect(function()
        ...
        for i = 0, 20 do
            ...
            local high = (low + 1) % n
            local frac = pos - low
            local currentColor = colors[low + 1]:Lerp(colors[high + 1], frac)
...
local Settings = { Range = 5000, AttackSpeed = 0.005, AutoScanRemotes = true }
...
task.spawn(function()
    while true do
        ...
        if _G.G_FastAttackMode == "模式1" then
             ...
             local waitTime = math.max(0.005 - elapsed, 0.001)
             task.wait(waitTime)
        end
    end
end)
...
RJR[L("主要功能")]:Button({
    ...
    Callback = function()
        pcall(function()
            loadstring(game:HttpGet("https://github.com/AGlizipy/fly1/releases/download/lajifly/fly.lua"))()
        end)
    end
})
RJR["ESP"]:Toggle({
